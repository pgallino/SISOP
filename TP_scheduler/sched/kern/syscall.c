...
// Returns current environment's priority
static int
...
// Chequea que la prioridad sea válida (Solo se puede disminuir), y se la cambia al proceso.
// Devuelve 0 si no hubo errores.
static int
sys_change_priority(envid_t env_id, int priority)
{
	struct Env *env;
	int error;

	if (priority < MIN_PRIOR) {
		return -E_INVAL;
	}

	if ((error = envid2env(env_id, &env, 1))) {
		return error;
	}

	if (env->priority > priority) {  // The priority can only be decreased
		return -E_INVAL;
	}
